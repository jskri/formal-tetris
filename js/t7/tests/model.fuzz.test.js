...
function randHolesFn() {
  return () => Math.floor(Math.random() * TI.InitialMainGrid[0].length);
}
...
function randomStep(machines) {
  ...
  if (kind === 0) m.movePiece(randOf([-1, 0]), randOf([-1, 0, 1]));
    ...
    /* disconnect: no Machine method — oracle-only, driven separately */
...
describe('T7 model.js fuzz', () => {
  test('long random multi-instance traces: no throw + T6-level invariants + T7-level bounds', () => {
    for (let trial = 0; trial < 15; trial++) {
      ...
      for (let pl = 0; pl < N; pl++)
        machines[pl] = new eng.Machine(pl, makeBagsFn());
      for (let step = 0; step < 800; step++) {
        ...
        for (const m of machines) {
          ...
          assert.ok(
            Number.isInteger(m.target) && m.target >= 0 && m.target < N,
          );
  ...
  test('differential oracle agreement over random multi-instance traces, all event kinds incl. disconnect', () => {
    ...
    for (let trial = 0; trial < 15; trial++) {
      ...
      const machines = [];
      for (let pl = 0; pl < N; pl++)
        machines[pl] = new eng.Machine(pl, bagsFns[pl]);
      ...
      for (let step = 0; step < 200; step++) {
        ...
        if (kind === 0) {
          ...
          const hole = Math.floor(Math.random() * TI.InitialMainGrid[0].length);
        ...
        for (let p2 = 0; p2 < N; p2++) {
          assertSnapshotsEqual(
            eng.snapshot(machines[p2]),
            oracle7.snapshotOf7(s, p2),
            `trial ${trial} step ${step} player ${p2}`,
          );
        }
      }
    }
  ...
  test('adversarial checkAxioms (PlayerCount and HostIndex, delegated through T6-T1)', () => {
    ...
    try {
      ...
      assert.strictEqual(
        tripped,
        true,
        'a ragged main grid must still trip a T1-level axiom',
      );

      tripped = false;
      const singlePlayerEng = T(
        TI.Piece,
        TI.InitialMainGrid,
        TI.ForbiddenGrid,
        TI.RotGrid,
      ...
      assert.strictEqual(
        tripped,
        true,
        'PlayerCount = 1 must trip AxiomsPlayer',
      );
    } finally {
