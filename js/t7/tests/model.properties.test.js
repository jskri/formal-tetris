...
const eventArb = fc.oneof(
  fc.record({
    ...
    pl: plArb,
    dy: fc.constantFrom(-1, 0),
    dx: fc.constantFrom(-1, 0, 1),
  }),
  fc.record({ kind: fc.constant('rotate'), pl: plArb, cw: fc.boolean() }),
  fc.record({
    kind: fc.constant('fix'),
    pl: plArb,
    bagNew: bagArb,
    hole: fc.integer({ min: 0, max: 1 }),
  }),
  fc.record({
    kind: fc.constant('fall'),
    pl: plArb,
    bagNew: bagArb,
    hole: fc.integer({ min: 0, max: 1 }),
  }),
...
function makeBagsFnFromSeed(bag) {
  return () => bag;
}
function constHolesFn(hole) {
  return () => hole;
}
...
function applyEvent(machines, e) {
  ...
  switch (e.kind) {
    case 'move':
      return m.movePiece(e.dy, e.dx);
    case 'rotate':
      return m.rotatePiece(e.cw);
    case 'fix':
      return m.fixPiece(e.bagNew, constHolesFn(e.hole));
    case 'fall':
      return m.fallStep(e.bagNew, constHolesFn(e.hole));
    case 'hold':
      return m.holdPiece(e.bagNew);
    case 'drop':
      return m.dropPiece(e.bagNew, constHolesFn(e.hole));
    case 'kick':
      return m.rotateKickPiece(e.cw);
    case 'disconnect':
      return null; // no Machine method exists (§4-T7f) — oracle only
    case 'notice':
...
function applyEvent7(s, e, params) {
  switch (e.kind) {
    case 'move':
      return oracle7.movePiece7(e.pl, e.dy, e.dx, s, params);
    case 'rotate':
      return oracle7.rotatePiece7(e.pl, e.cw, s, params);
    case 'fix':
      return oracle7.fixPiece7(e.pl, constHolesFn(e.hole), e.bagNew, s, params);
    case 'fall':
      return oracle7.fallStep7(e.pl, constHolesFn(e.hole), e.bagNew, s, params);
    case 'hold':
      return oracle7.holdPiece7(e.pl, e.bagNew, s, params);
    case 'drop':
      return oracle7.dropPiece7(
        e.pl,
        constHolesFn(e.hole),
        e.bagNew,
        s,
    ...
    case 'kick':
      return oracle7.rotateKickPiece7(e.pl, e.cw, s, params);
    case 'disconnect':
      return oracle7.disconnectPlayer7(e.pl, s, params);
    case 'notice':
      return oracle7.noticeDisconnection7(e.pl, s);
  }
}
...
function assertAllSnapshotsAgree(machines, s) {
  for (let pl = 0; pl < N; pl++) {
    assertSnapshotsEqual(
      eng.snapshot(machines[pl]),
      oracle7.snapshotOf7(s, pl),
      `mismatch for player ${pl}`,
    );
  }
}
...
describe('T7 model.js properties', () => {
    ...
    fc.assert(
      fc.property(
        ...
        (initBag, events) => {
          const machines = [];
          for (let pl = 0; pl < N; pl++)
            machines[pl] = new eng.Machine(pl, makeBagsFnFromSeed(initBag));
          for (const e of events) {
            ...
            assert.ok(
              winners <= 1,
              'at most one player may believe herself the winner at a time',
            );
          }
        },
      ),
  ...
  test("differential oracle: every player's snapshot agrees at every step, all event kinds incl. messages/disconnect", () => {
    ...
    fc.assert(
      fc.property(
        bagArb,
        fc.array(eventArb, { maxLength: 80 }),
        (initBag, events) => {
          const machines = TI.Player.map(
            (pl) => new eng.Machine(pl, makeBagsFnFromSeed(initBag)),
          );
          let s = oracle7.initState7(
            TI.Player.map(() => makeBagsFnFromSeed(initBag)),
            params,
          );
