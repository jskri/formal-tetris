...
function applyEvent(m, e) {
  switch (e.kind) {
    ...
    case 'fix':
      return m.fixPiece(e.pNew);
    case 'fall':
      return m.fallStep(e.pNew);
  }
}
...
function applyEvent2(rs, e, params) {
  switch (e.kind) {
    case 'move':
      return oracle2.movePiece(e.dy, e.dx, rs, params);
    case 'rotate':
      return oracle2.rotatePiece(e.cw, rs, params);
    case 'fix':
      return oracle2.fixPiece(e.pNew, rs, params);
    case 'fall':
      return oracle2.fallStep(e.pNew, rs, params);
  }
}
      ...
      fc.property(
        ...
        (p0, events) => {
          ...
          for (const e of events) {
            ...
            assert.strictEqual(
              m.level,
              1 + Math.floor(m.totalClearedLines / 10),
            );
    ...
    fc.assert(
      fc.property(
        ...
        (clearedLines, level, combo, perfectClear) => {
          ...
          const expectedPc = perfectClear
            ? (pcTable[clearedLines] ?? 2000) * level
            : 0;
          assert.strictEqual(
            eng.points(clearedLines, level, combo, perfectClear),
