...
export function T(
  ...
  function checkInvariants(s, message, isOccupied = (c) => Piece.includes(c)) {
    ...
    // LevelCorrect
    console.assert(
      s.level === 1 + Math.floor(s.totalClearedLines / 10),
      `LevelCorrect failed @ ${message}`,
    );
    console.assert(s.combo >= 0, `combo >= 0 failed @ ${message}`);
    console.assert(
      s.totalClearedLines >= 0,
      `totalClearedLines >= 0 failed @ ${message}`,
    );
    ...
    for (const f of [s.score, s.level, s.combo, s.totalClearedLines])
      console.assert(
        ...
        `safe-integer field failed @ ${message}`,
      );
  }
  ...
  class Machine {
    ...
    fixPiece(pNew) {
      ...
      const combo2 =
        clearedLines === 0 ? 0 : capAdd(this.combo, 1, MAX_SAFE_INTEGER); // 4
