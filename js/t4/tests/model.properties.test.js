...
const bagArb = fc.shuffledSubarray(TI.Piece, {
  minLength: TI.Piece.length,
  maxLength: TI.Piece.length,
});
...
function applyEvent(m, e) {
  switch (e.kind) {
    ...
    case 'fix':
      return m.fixPiece(e.bagNew);
    case 'fall':
      return m.fallStep(e.bagNew);
    case 'hold':
...
function applyEvent4(rs, e, params) {
  switch (e.kind) {
    case 'move':
      return oracle4.movePiece4(e.dy, e.dx, rs, params);
    case 'rotate':
      return oracle4.rotatePiece4(e.cw, rs, params);
    case 'fix':
      return oracle4.fixPiece4(e.bagNew, rs, params);
    case 'fall':
      return oracle4.fallStep4(e.bagNew, rs, params);
    case 'hold':
      return oracle4.holdPiece4(e.bagNew, rs, params);
  }
}
...
describe('T4 model.js properties', () => {
  test('bag_ prefix validity, next_ well-formedness, T3 invariants hold after every step', () => {
    fc.assert(
      fc.property(
        bagArb,
        fc.array(eventArb, { maxLength: 150 }),
        (initBag, events) => {
  ...
  test('fixPiece guard-fail leaves bag_/next_ untouched (peek-then-commit)', () => {
    fc.assert(
      fc.property(bagArb, bagArb, (initBag, bagNew) => {
        const m = new eng.Machine(makeBagsFnFromSeed([initBag]));
        const bagBefore = m.bag_.slice(),
          nextBefore = m.next_.slice();
