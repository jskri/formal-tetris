...
const constants = {
  ...
  PW, FY, FX, NextLen,
  Piece, // needed by view.js's effectiveRowRange (preview-slot sizing)
  FH: ForbiddenGrid.length,
  FW: ForbiddenGrid[0].length,
  rotGrid: eng.T3Eng.T2Eng.T1Eng.rotGrid,
  PANEL_PX: 0, // set by sizeCanvas before the first frame (per-side width)
};
...
// spec: bagNew — a fresh permutation of the full Piece array (Fisher–Yates).
// Replaces T1–T3's randomPiece() entirely: nothing in T4's controller needs a
// single uniformly-random piece.
function shuffleBag() {
  const a = Piece.slice();
...
// bagsFn: memoizes each newly-requested index exactly once, satisfying
// initPieceAndDraw's referential-consistency requirement.
function makeBagsFn() {
  const bags = [];
