...
export function T(
  ...
  function assertPieceSet(arr) {
    if (!isPieceSet(arr))
      throw new Error(
        `assertPieceSet: not a valid piece set: ${JSON.stringify(arr)}`,
      );
  }
  ...
  function checkAxioms() {
    T3Eng.checkAxioms();
    console.assert(
      Number.isInteger(NEXT_LEN) && NEXT_LEN > 0,
  ...
  function checkInvariants(s, message, isOccupied = (c) => Piece.includes(c)) {
    ...
    console.assert(
      s.bag_.length <= NUM_PIECES,
      `bag_ length bound failed @ ${message}`,
    );
    console.assert(
      new Set(s.bag_).size === s.bag_.length,
      `bag_ elements not pairwise distinct @ ${message}`,
    );
    console.assert(
      s.bag_.every((p) => Piece.includes(p)),
      `bag_ ⊆ Piece failed @ ${message}`,
    );
    console.assert(
      s.next_.length === NEXT_LEN,
      `next_ length failed @ ${message}`,
    );
    console.assert(
      s.next_.every((p) => Piece.includes(p)),
      `next_ ⊆ Piece failed @ ${message}`,
    );
  }
  ...
  class Machine {
    ...
    get level() {
      return this.s3.level;
    }
    get totalClearedLines() {
      return this.s3.totalClearedLines;
    }
    get combo() {
      return this.s3.combo;
    }
    get perfectClear() {
      return this.s3.perfectClear;
    }
    get score() {
      return this.s3.score;
    }
    ...
    holdPiece(bagNew) {
      ...
      const p2 =
        this.s3.hold !== null ? this.s3.hold : this.drawNextPiece(bagNew);
