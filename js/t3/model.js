...
export function T(
  ...
  function checkInvariants(s, message, isOccupied = (c) => Piece.includes(c)) {
    ...
    console.assert(
      !s.swapped || s.hold !== null,
      `SwappedImplyHoldSome failed @ ${message}`,
    );
    console.assert(
      !s.gameover || !s.swapped,
      `GameoverImplyNotSwapped failed @ ${message}`,
    );
    console.assert(
      s.hold === null || Piece.includes(s.hold),
  ...
  class Machine {
    ...
    get level() {
      return this.s2.level;
    }
    get totalClearedLines() {
      return this.s2.totalClearedLines;
    }
    get combo() {
      return this.s2.combo;
    }
    get perfectClear() {
      return this.s2.perfectClear;
    }
    get score() {
      return this.s2.score;
    }
