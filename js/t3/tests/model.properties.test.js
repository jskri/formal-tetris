...
function applyEvent3(rs, e, params) {
  switch (e.kind) {
    case 'move':
      return oracle3.movePiece3(e.dy, e.dx, rs, params);
    case 'rotate':
      return oracle3.rotatePiece3(e.cw, rs, params);
    case 'fix':
      return oracle3.fixPiece3(e.pNew, rs, params);
    case 'fall':
      return oracle3.fallStep3(e.pNew, rs, params);
    case 'hold':
      return oracle3.holdPiece3(e.pNew, rs, params);
  }
}
...
describe('T3 model.js properties', () => {
  ...
  test('holdPiece narrow blast radius: only p/py/px/pr/hold/swapped ever change on a firing hold', () => {
    fc.assert(
      fc.property(pieceArb, pieceArb, (p0, pNew) => {
        const m = new eng.Machine(p0);
        ...
        for (const f of [
          ...
          if (f === 'mg')
            assert.ok(gridsEqual(after.mg, before.mg), 'mg mismatch');
