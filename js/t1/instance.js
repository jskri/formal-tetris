...
export const InitialMainGrid = Array.from({ length: 22 }, () =>
  Array(10).fill(false),
);
export const ForbiddenGrid = Array.from({ length: 2 }, () =>
  Array(10).fill(true),
);

export function InitialY(_p) {
  return 19;
}
export function InitialX(_p) {
  return 3;
}
...
for (const p of Object.keys(GRIDS)) {
  ROT_GRID_CACHE[p] = GRIDS[p].map((g) =>
    g.map((row) => row.map((c) => (c === true ? p : false))),
  );
}
