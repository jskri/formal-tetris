...
export function T(
  ...
  function bboxInsideBBox(g1, y1, x1, g2, y2, x2) {
    return (
      y2 <= y1 &&
      y1 + g1.length - 1 <= y2 + g2.length - 1 &&
      x2 <= x1 &&
      x1 + g1[0].length - 1 <= x2 + g2[0].length - 1
    );
  }
  ...
  function occupiedInside(g1, y1, x1, g2, y2, x2) {
    return g1.every((row, y) =>
      row.every((cell, x) => {
        ...
        return (
          !occ(cell) ||
          (0 <= oy && oy < g2.length && 0 <= ox && ox < g2[0].length)
        );
      }),
    );
  }
  ...
  function valid(g, p, py, px, pr) {
    ...
    return (
      occupiedInside(rg, py, px, g, 0, 0) && !intersect(rg, py, px, g, 0, 0)
    );
  }
  ...
  function resize(g, newGh, fill) {
    ...
    return Array.from({ length: newGh }, (_, y) =>
      y < g.length
        ? g[y].slice()
        : Array.from({ length: w }, (_, x) => fill(x)),
    );
  }
  ...
  function checkAxioms() {
    // AxiomsPW
    console.assert(
      Number.isInteger(PW) && PW > 0,
      'AxiomsPW: PW must be a positive integer',
    );

    // D8: Piece is a finite, complete, primitive enumeration
    console.assert(
      Array.isArray(Piece) && Piece.length > 0,
      'D8: Piece must be a non-empty array',
    );
    ...
    for (const p of Piece) {
      ...
      console.assert(
        1 - PW <= iy && iy < HM,
        `AxiomsInitialYX: InitialY(${String(p)}) out of range`,
      );
      console.assert(
        1 - PW <= ix && ix < WM,
        `AxiomsInitialYX: InitialX(${String(p)}) out of range`,
      );
    }
    ...
    for (const p of Piece) {
      for (let r = 0; r < 4; r++) {
        ...
        console.assert(
          gr.length === PW && gr.every((row) => row.length === PW),
        ...
        console.assert(
          occupied,
          `AxiomsRotGrid: rotGrid(${String(p)},${r}) must have >=1 occupied cell`,
        );
      }
      console.assert(
        fullyContainedIn(
    ...
    console.assert(
      HM > 0,
      'AxiomsInitialMainGrid: H InitialMainGrid must be > 0',
    );
    console.assert(
      WM > 0,
      'AxiomsInitialMainGrid: W InitialMainGrid must be > 0',
    );
    console.assert(
    ...
    // AxiomsForbiddenGrid
    console.assert(
      ForbiddenGrid.length > 0,
      'AxiomsForbiddenGrid: H ForbiddenGrid must be > 0',
    );
    console.assert(
      ForbiddenGrid[0].length > 0,
      'AxiomsForbiddenGrid: W ForbiddenGrid must be > 0',
    );
    console.assert(FY >= 0, 'AxiomsForbiddenGrid: FY >= 0');
    console.assert(FX >= 0, 'AxiomsForbiddenGrid: FX >= 0');
    console.assert(
      bboxInsideBBox(ForbiddenGrid, FY, FX, InitialMainGrid, 0, 0),
    ...
    console.assert(
      ...
      'D10: MAX_SAFE_INTEGER must be an integer',
    );
    ...
    const dims = [
      ...
      FY,
      FX,
      ...Piece.flatMap((p) => [InitialY(p), InitialX(p)]),
    ];
    console.assert(
      dims.every((v) => Math.abs(v) <= MAX_SAFE_INTEGER),
  ...
  function typeOK(s, isOccupied = (c) => Piece.includes(c)) {
    return (
      ...
      Number.isInteger(s.py) &&
      Number.isInteger(s.px) &&
      Number.isInteger(s.pr) &&
      0 <= s.pr &&
      s.pr <= 3 &&
      typeof s.gameover === 'boolean' &&
      Number.isInteger(s.clearedLines) &&
      s.clearedLines >= 0 &&
      Piece.includes(s.p)
    );
  }
  ...
  function checkInvariants(s, message, isOccupied = (c) => Piece.includes(c)) {
    ...
    // Gameover
    console.assert(
      s.gameover === intersect(ForbiddenGrid, FY, FX, s.mg, 0, 0),
      `Gameover invariant failed @ ${message}`,
    );
    // PieceOccupiedInsideBounds (unconditional)
    console.assert(
      occupiedInside(rotGrid(s.p, s.pr), s.py, s.px, s.mg, 0, 0),
      `PieceOccupiedInsideBounds failed @ ${message}`,
    );
    // PieceOnFreeBlocks (guarded by !gameover)
    console.assert(
      s.gameover || !intersect(rotGrid(s.p, s.pr), s.py, s.px, s.mg, 0, 0),
      `PieceOnFreeBlocks failed @ ${message}`,
    );
    // NoFullLine
    console.assert(
      s.mg.every((_, y) => !isFullLineb(s.mg, y)),
  ...
  class Machine {
    ...
    movePiece(dy, dx) {
      ...
      if (!(!this.gameover && canMovePiece(dy, dx, this))) return false; // req-piece-move-dir
      this.py = py2;
      this.px = px2;
    ...
    // spec: FixPiece p_new
    fixPiece(pNew) {
      if (!(!this.gameover && !canMovePiece(-1, 0, this))) return false; // real guard
      if (CHECK_INVARIANTS)
        console.assert(Piece.includes(pNew), 'pNew in Piece'); // D5
      // simultaneous-assignment order (skill §4b, proofs.md §9):
      const u = union(
        this.mg,
        0,
        0,
        rotGrid(this.p, this.pr),
        this.py,
        this.px,
      ); // 1: reads pre-state
